"""
Long Call Strategy for Probability of Profit calculations.
"""
...
def _bsm_long_call(sim_price: np.ndarray, strikes: np.ndarray, rate: float,
                   time_fraction: float, sigma: float) -> np.ndarray:
    """BSM debit function for long call."""
...
class LongCall:
    ...
    Profit occurs when stock rises above strike + premium paid.
    ...
    def __init__(
        ...
        """
        Initialize Long Call.

        Args:
            strike: Strike price of the call
            premium: Premium paid for the call
        """
        self.strike = strike
