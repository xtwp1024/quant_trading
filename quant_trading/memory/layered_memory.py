"""
LayeredMemory: top-level orchestration of the FinMem layered memory architecture.

Manages four memory layers with distinct cognitive roles:

    PERCEPTUAL  -- Raw market stimuli: price ticks, news headlines, economic prints
                   Highest decay rate. Minimal retention. Filters sensory noise.
    SHORT_TERM   -- Recent trading memories: today's positions, recent signals
                   High recency weight. Medium decay. Acts as working memory.
    MID_TERM    -- Cross-day patterns: multi-day trends, sector rotations
                   Balanced importance and recency. Bridge between short and long.
    LONG_TERM   -- Enduring knowledge: verified strategies, fundamental models
                   High importance weight. Slowest decay. Core professional expertise.

Layer transitions (memory jump):
    Short --(promote high-importance)--> Mid
    Short <--(demote low-importance)-- Mid
    Mid   --(promote high-importance)--> Long
    Mid   <--(demote low-importance)-- Long
    Perceptual --> Short  (new stimuli enter here)

Each layer has independently tunable cognitive span parameters, allowing
retention beyond human perceptual limits (e.g., remembering every tick
of the past 30 days with full fidelity).

The reflection layer (not modeled here) synthesizes insights from
accumulated memories -- this is a natural extension point for a
future quant_trading.memory.reflector module.
"""
...
class LayeredMemory:
    ...
    Coordinates four memory layers with self-evolving transitions.
    All layers share the same symbol namespace (e.g., "AAPL", "BTC-USD").

    Example:
        >>> lm = LayeredMemory(symbols=["AAPL", "TSLA"])
        >>> lm.add_memory("AAPL", "Q4 earnings beat expectations by 12%", layer="perceptual")
        >>> lm.add_memory("TSLA", "Short-term overbought RSI=78", layer="short")
        >>> results = lm.query("AAPL", top_k=3, layer="short")
        >>> lm.step()  # advance time: decay + cleanup + jumps
    """

    # -------------------------------------------------------------------------
    # Default cognitive span parameters per layer
    # These encode how long information persists before natural decay
    # -------------------------------------------------------------------------
    ...
    def __init__(
        ...
        # Optional semantic search overrides per layer
        ...
        Args:
            ...
            semantic_query_fns:  Optional semantic search functions per layer:
                                   {"short": fn, "mid": fn, ...}
        """
        self._logger = logger or logging.getLogger("LayeredMemory")
        ...
        for layer in ["perceptual", "short", "mid", "long"]:
            p = params[layer]
            self._layers[layer] = MemoryDatabase(
                db_name=f"layered_memory_{layer}",
                layer=layer,
        ...
        self._logger.info(
            f"LayeredMemory initialized with symbols={symbols}, "
    ...
    def add_memory(
        ...
        Args:
            symbol:    Asset ticker.
            text:      Memory content.
            layer:     Target layer: "perceptual", "short", "mid", or "long".
    ...
    def step(self) -> Dict[str, List[int]]:
        ...
        For each layer:
            ...
            3. Execute memory jumps (promotion/demotion) between layers
        ...
        # Phase 2: Execute jumps (two passes for bidirectional flow)
        # Pass 1: short -> mid (up), mid -> long (up), short -> mid (down)
        # Pass 2: mid -> short (down), long -> mid (down), mid -> long (down)

        # Short -> Mid promotion
        jump_up, jump_down, _ = jump_dicts["short"]
        self._layers["mid"].accept_jump(jump_up, direction="up")

        # Short -> Mid demotion (low importance leaves short-term)
        ...
        self._logger.debug(
            f"step() removed totals: perceptual={len(removed['perceptual'])}, "
            f"short={len(removed['short'])}, mid={len(removed['mid'])}, "
            f"long={len(removed['long'])}"
        )
    ...
    def __repr__(self) -> str:
        total = sum(len(db) for db in self._layers.values())
        layer_counts = {layer: len(db) for layer, db in self._layers.items()}
        return (
            f"<LayeredMemory total_entries={total} "
